...
void readSensors()
{
  ...
  sensor0 = getSensorValue(3);  // p-thumb
...
{   
  ...
  if(sensorValue > mMinMax[sensorPin][1]) {mMinMax[sensorPin][1] = sensorValue;}  // set max  
    ...
    Serial.println("runtime minmax");
  Serial.println(mMinMax[sensorPin][0]);
  Serial.println(mMinMax[sensorPin][1]);
...
//  int min=40, max=100;//max=869;
//  if(sensorValue<min) sensorValue=min;
//  else if(sensorValue>max) sensorValue=max;
//  float ratio=180.0/(max-min);
//  sensorValue = (sensorValue-min)*ratio;
...
{   // map sensor value to servo position
  int value =  map(sensorValue, mSENSOR_MIN, mSENSOR_MAX, mSERVO_MIN, mSERVO_MAX);
  Serial.println("mapped servo");
...
int smooth(int sensorValue, int sensorPin)
{
  ...
  {
    if(mSensorSmoothing[sensorPin][i]!=0)
    {
      average += mSensorSmoothing[sensorPin][i];
      count++;
    }
  }

  sensorValue = average/count;
